...
st.title("Indic Vision UI - OCR for Indian Languages")
...
if is_image_uploaded:
    ...
    tooltip_markdown = '''
        Select the Indic language to perform OCR on
    '''.strip()
    ...
    if languages_selected:
        all_languages_selected = st.sidebar.button("Perform OCR on the Indic language")
    ...
    if all_languages_selected:
        with st.expander("Perform OCR", expanded=True):            
            ...
            with st.spinner("It takes some time to perform OCR if you are on a CPU, this module is much faster on a GPU"):
                ...
                st.success("OCR on the Indic language completed successfully!")
                st.balloons()
